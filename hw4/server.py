...
logger = logging_client.logger("hw4-webserver")
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if country in BANNED_COUNTRIES:
            logger.log_struct(
                {"message": f"Forbidden request from {country}", "ip": client_ip, "path": path},
                severity="CRITICAL"
            )
    ...
    def _send_501(self):
        client_ip = self.client_address[0]
        logger.log_struct(
            {"message": f"Unsupported method: {self.command}", "ip": client_ip},
            severity="WARNING"
        )
